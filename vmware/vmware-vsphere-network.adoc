...
* Tutte le versioni di VMware vSphere attualmente supportate possono utilizzare sia NFS v3 che v4,1. Il supporto ufficiale per nconnect è stato aggiunto a vSphere 8,0 update 2 per NFS v3. Per NFS v4,1, vSphere continua a supportare il trunking della sessione, l'autenticazione Kerberos e l'autenticazione Kerberos con integrità. È importante notare che il trunking della sessione richiede ONTAP 9.14.1 o una versione successiva.
...
** Utilizza i tool ONTAP per VMware vSphere per il provisioning dei datastore, perché semplifica la gestione automatica delle policy di esportazione.
...
** Utilizzare la funzione di montaggio plug-in per applicare i datastore esistenti ai nuovi server.
