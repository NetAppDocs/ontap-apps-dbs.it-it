...
A partire da ONTAP 9.19.1, la connessione FC diretta è supportata sia con FC-SAN tradizionale (SCSI) che con NVMe/FC con specifici adattatori FC del sistema storage. Gli HBA stabiliranno una connessione FC punto-punto standard. Prima di ONTAP 9.19.1, i sistemi storage ONTAP richiedono NPIV, il che significa che il sistema storage deve essere connesso a uno switch FC che supporta anch'esso NPIV.
