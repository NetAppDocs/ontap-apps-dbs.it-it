...
* Utilizzare una singola interfaccia logica (LIF) per ogni SVM su ciascun nodo nel cluster ONTAP. Le precedenti raccomandazioni di una LIF per ogni datastore non sono più necessarie. Sebbene l'accesso diretto (LIF e datastore sullo stesso nodo) sia la soluzione migliore, non preoccuparti dell'accesso indiretto perché l'effetto sulle prestazioni è generalmente minimo (microsecondi).
...
[TIP]
====
* Il valore massimo per nconnect in vSphere 8 è 4 e il valore predefinito è 1. Il limite del valore massimo in vSphere può essere aumentato per ogni singolo host tramite le impostazioni avanzate, sebbene in genere non sia necessario.
...
* Tieni presente che ESXi ha un limite di 256 connessioni NFS e ogni connessione nconnect contribuisce a tale totale. Ad esempio, due datastore con nconnect=4 verrebbero conteggiati come otto connessioni totali.
* È importante testare l'impatto sulle prestazioni di nconnect nel proprio ambiente prima di implementare modifiche su larga scala negli ambienti di produzione.
...
* Come menzionato in link:vmware-vsphere-settings.html["impostazioni"], se non stai utilizzando il vSphere CSI per Kubernetes, dovresti impostare il newSyncInterval per  https://knowledge.broadcom.com/external/article/386364/reducing-excessive-vsan-cnssync-warnings.html["VMware KB 386364"^]
...
* Sebbene ONTAP offra una struttura di namespace per i volumi flessibile per organizzare i volumi in una struttura ad albero utilizzando le giunzioni, questo approccio non ha valore per vSphere. Crea una directory per ogni VM nella radice del datastore, indipendentemente dalla gerarchia del namespace dello storage. Pertanto, la best practice consiste semplicemente nel montare il percorso di giunzione per i volumi per vSphere sul volume root della SVM, che è il modo in cui ONTAP tools for VMware vSphere esegue il provisioning dei datastore. Non avere percorsi di giunzione annidati significa anche che nessun volume dipende da alcun volume diverso dal volume root e che mettere offline o distruggere un volume, anche intenzionalmente, non influisce sul percorso verso gli altri volumi.
