...
NOTE: vVols richiede l'utilizzo di ONTAP tools for VMware vSphere, che funziona come VASA Provider per ONTAP. Consulta link:vmware-vvols-overview.html["Volumi virtuali di VMware vSphere (vVol) con ONTAP"] per le best practice di vVols.
