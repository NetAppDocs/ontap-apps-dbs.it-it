...
vSphere include il supporto integrato per percorsi multipli verso i dispositivi di storage. vSphere può rilevare il tipo di dispositivo di storage per i sistemi storage supportati e configura automaticamente lo stack multipathing per supportare le funzionalità del sistema storage in uso, indipendentemente dal protocollo utilizzato o se si utilizza ASA, AFF, FAS o software-defined ONTAP.
...
NOTE: Consultare la link:https://configmax.broadcom.com/guest?vmwareproduct=vSphere&release=vSphere%208.0&categories=2-0["Strumento VMware Configuration Maximums"^] per i limiti più aggiornati supportati in ESXi.
...
* Per le reti iSCSI o NVMe/TCP, utilizzare più interfacce di rete VMkernel su diverse subnet di rete non instradate con NIC teaming quando sono presenti più switch virtuali. È anche possibile utilizzare più schede di rete fisiche connesse a più switch fisici per fornire HA e throughput aumentato. In ONTAP, configurare un gruppo di interfacce in modalità singola per il failover con due o più collegamenti connessi a due o più switch, oppure utilizzare LACP o altre tecnologie di link aggregation con gruppi di interfacce multimodali per fornire HA e i vantaggi della link aggregation. Tenere presente che NVMe/TCP deve essere abilitato per ogni porta VMkernel e per ogni interfaccia fisica, e occorre allinearli in modo che le porte VMkernel NVMe/TCP abbiano accesso alle schede di rete fisiche con NVMe/TCP abilitato. La figura seguente illustra la connettività multipath con protocolli SAN basati su IP. image:vsphere_ontap_image2.png["connettività multipath"]
