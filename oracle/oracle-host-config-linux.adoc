...
Un'opzione di montaggio aggiuntiva, denominata  `nosharecache`, è necessaria quando (a) DNFS è abilitato e (b) una condivisione NFS di origine è montata più di una volta su un singolo server (c) con un montaggio NFS nidificato. Questa configurazione si riscontra principalmente negli ambienti che supportano applicazioni SAP. Ad esempio, una singola condivisione NFS su un sistema NetApp potrebbe avere una directory situata a  `/vol/oracle/base` e una seconda a  `/vol/oracle/home`. Se  `/vol/oracle/base` è montata a  `/oracle` e  `/vol/oracle/home` è montata a  `/oracle/home`, il risultato sono montaggi NFS nidificati che hanno origine dalla stessa origine.

Il sistema operativo può rilevare che  `/oracle` e  `/oracle/home` risiedono sulla stessa condivisione NFS, ovvero sullo stesso file system. Il sistema operativo utilizza quindi lo stesso handle di dispositivo per accedere ai dati. In questo modo si migliora l'utilizzo della cache del sistema operativo e di alcune altre operazioni, ma si interferisce con DNFS. Se DNFS deve accedere a un file, come  `spfile`, su  `/oracle/home`, potrebbe erroneamente tentare di utilizzare il percorso sbagliato per i dati. Il risultato è un'operazione di I/O fallita. In queste configurazioni, aggiungere l'opzione di mount  `nosharecache` a qualsiasi file system NFS che condivide una condivisione NFS di origine con un altro file system NFS su quello stesso host. In questo modo si obbliga il sistema operativo Linux ad allocare un handle di dispositivo indipendente per quel file system.
