...
Il requisito più importante è l'isolamento dei datafile in uno o più container dedicati. Ciò può essere fatto in diversi modi, incluso il posizionamento di tutti i datafile su un singolo LUN/namespace, all'interno di un singolo volume AFF NFS, oppure in un gruppo di coerenza ONTAP multi-volume o multi-LUN/namespace. Il punto chiave è che i datafile non devono essere contaminati da altri tipi di file. Il motivo è garantire che la replica dei datafile sia completamente indipendente dalla replica di altri tipi di dati, come gli archive log. Per ulteriori informazioni sui layout dei file e per dettagli importanti su come garantire che il layout di storage sia compatibile con la tecnologia Snapshot, vedere link:oracle-dp-online-backup.html#data-layout["Layout dei dati"].
