...
Durante le operazioni di manutenzione ordinaria, un'operazione di takeover dello storage o di switchover MetroCluster dovrebbe essere trasparente, salvo una potenziale breve pausa nelle operazioni mentre i percorsi di rete cambiano. La rete può essere complessa, tuttavia, ed è facile commettere errori, quindi NetApp raccomanda vivamente di testare accuratamente le operazioni di takeover e switchover prima di mettere un sistema storage in produzione. Farlo è l'unico modo per essere certi che tutti i percorsi di rete siano configurati correttamente. In un ambiente SAN, verificare attentamente l'output del comando  `sanlun lun show -p` per assicurarsi che tutti i percorsi primari e secondari previsti siano disponibili.
