...
summary: Configurazione di Oracle e TCP/IP ed Ethernet sui sistemi ASA r2 
searchtitle: Configurazione TCP/IP ed Ethernet per database Oracle con ASA r2 
---
...
TIP: * NetApp consiglia* di assicurarsi che i controller di storage NetApp ASA r2 non ricevano pacchetti di controllo del flusso Ethernet. In genere, questa operazione può essere eseguita impostando le porte dello switch a cui è collegato il controller, ma alcuni hardware dello switch presentano delle limitazioni che potrebbero richiedere modifiche lato client.
