...
A titolo di esempio, un sistema AFF A800 HA a due nodi di vecchia generazione è in grado di gestire fino a un milione di IOPS casuali prima che la latenza superi un millisecondo. È improbabile che singoli carichi di lavoro raggiungano tali livelli. Sfruttare appieno questo array AFF A800 comporta l'hosting di più carichi di lavoro e, per farlo in modo sicuro garantendo la prevedibilità, sono necessari controlli QoS.

Esistono due tipi di quality of service (QoS) in ONTAP: IOPS e larghezza di banda. I controlli QoS possono essere applicati a AFF SVM, AFF volumi, AFF file e LUN e namespace AFF/ASA.
