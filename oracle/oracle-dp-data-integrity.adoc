...
Per proteggere i dati da questa situazione, ONTAP consente di configurare volumi, LUN o namespace NVMe con una protezione speciale contro i guasti della NVRAM. Quando si attiva, questo meccanismo di protezione fa sì che le risorse di storage entrino in uno stato chiamato NVFAIL. Questo stato provoca errori di I/O che causano l'arresto dell'applicazione per evitare che utilizzi dati obsoleti. I dati non dovrebbero andare persi perché ogni scrittura confermata dovrebbe essere presente sull'array di storage.

Di solito, i passaggi successivi prevedono che un amministratore arresti completamente gli host prima di rimettere manualmente online le LUN, i namespace o i volumi NFS. Sebbene questi passaggi possano richiedere del lavoro, questo approccio è il modo più sicuro per garantire l'integrità dei dati. Non tutti i dati necessitano di questa protezione, motivo per cui il comportamento di NVFAIL può essere configurato a livello granulare.
