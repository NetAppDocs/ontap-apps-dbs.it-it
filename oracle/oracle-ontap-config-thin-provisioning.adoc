...
Ad esempio, si consideri un database su una singola LUN da 250GB in un volume da 1TB su un sistema AFF. La creazione di uno snapshot comporterebbe immediatamente la prenotazione di ulteriori 250GB di spazio nel volume per garantire che il volume non si esaurisca per nessun motivo. L'utilizzo di riserve frazionarie è generalmente uno spreco, poiché è estremamente improbabile che ogni byte nel volume del database debba essere sovrascritto. Non c'è motivo di riservare spazio per un evento che non si verifica mai. Tuttavia, se un cliente non può monitorare il consumo di spazio in un sistema storage e deve essere certo che lo spazio non si esaurisca mai, per utilizzare gli snapshot sarebbero necessarie prenotazioni frazionarie al 100%.


TIP: L'astrazione del volume non è presente su SAN, ma solo su ASA, e pertanto questa piattaforma non utilizza riserve frazionarie.
...
Esiste sempre la possibilità che la comprimibilità di un dataset cambi, il che comporterebbe un aumento del consumo di spazio reale. Questo aumento del consumo significa che la compressione deve essere gestita come con altre forme di thin provisioning in termini di monitoraggio e, su un sistema AFF, utilizzando  `volume-autogrow` e  `snapshot-autodelete`.
...
[TIP]
====
*NetApp consiglia* le seguenti configurazioni di riserva con le piattaforme AFF e FAS:
