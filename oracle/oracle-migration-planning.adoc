...
Ad esempio, la copia di un database da 10TB richiede in genere circa sette ore per essere completata. Se le esigenze aziendali consentono un'interruzione di sette ore, la copia dei file è un'opzione semplice e sicura per la migrazione. Se cinque ore non sono accettabili, è possibile impostare con il minimo sforzo un semplice processo di log shipping (vedi link:oracle-migration-log-shipping.html["Log shipping di Oracle"]) per ridurre il tempo di cutover a circa 15 minuti. Durante questo tempo, un database administrator può completare il processo. Se anche 15 minuti non sono accettabili, il processo finale di cutover può essere automatizzato tramite script per ridurre il tempo di cutover a pochi minuti. Puoi sempre velocizzare una migrazione, ma farlo comporta un costo in termini di tempo e impegno. Gli obiettivi relativi al tempo di cutover dovrebbero basarsi su ciò che è accettabile per l'azienda.
