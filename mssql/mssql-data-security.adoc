...
A partire da ONTAP 9.12.1, gli snapshot non sono solo di sola lettura, ma possono anche essere protetti da cancellazioni accidentali o intenzionali. La funzionalità si chiama tamperproof snapshots. È possibile impostare e applicare un periodo di conservazione tramite la policy degli snapshot. Gli snapshot risultanti non possono essere eliminati fino al raggiungimento della data di scadenza. Non sono previste eccezioni da parte degli amministratori o del centro di supporto.
