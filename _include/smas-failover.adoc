...
I diagrammi mostrano cosa accade durante le operazioni normali, di failover e di failback. Per semplicità di illustrazione, rappresentano una LUN replicata. In una configurazione SnapMirror active sync reale, la replica si basa su volumi AFF, dove ogni volume contiene una o più LUN, ma per semplificare la rappresentazione, il livello dei volumi è stato omesso.
